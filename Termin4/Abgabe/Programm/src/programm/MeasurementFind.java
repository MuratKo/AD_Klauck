...
//		File file = new File(PATH + "messungFindL.csv");
//		file.createNewFile();
...
//		File file = new File(PATH + "messungFindQ.csv");
//		file.createNewFile();
//		PrintWriter writer = new PrintWriter(file);
//		
//		writer.write("500;1000;2000;4000;8000;16000;32000;64000;\n");
...
//		writer.close();
		...
		File file = new File(PATH + "messungFindB.csv");
		file.createNewFile();
		PrintWriter writer = new PrintWriter(file);
	...
	private static void messung(int anzahl, Strategy strategy, PrintWriter writer, ADTHashmapMeasurement map) throws TextNotSplittedException, IOException {
		...
		map = map.insert("dolore"); // Befindet sich nicht in der Text-Datei
		int anzahlVonDolore = map.find("dolore");
		timeFind = map.getTimeFind();
		DecimalFormat df = new DecimalFormat("#.00"); 
		//String timeInMilli = df.format(Double.valueOf(timeFind) / 1000000);
		writer.write(timeFind + ";");
	}
